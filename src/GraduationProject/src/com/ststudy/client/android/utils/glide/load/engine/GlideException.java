...
public final class GlideException extends Exception {
	...
	private static final long serialVersionUID = 8730941793144393597L;
	 ...
	 *
	 *      <p>
	 *      Causes may or may not be {@link GlideException GlideExceptions}.
	 *      Causes may also not be root causes, and in turn my have been caused
	 *      by other failures.
	 */
	...
	/**
	 * Returns the list of root causes that are the leaf nodes of all children
	 * of this exception.
	 *
	 * <p>
	 * Use this method to do things like look for http exceptions that indicate
	 * the load may have failed due to an error that can be retried. Keep in
	 * mind that because Glide may attempt to load a given model using multiple
	 * different pathways, there may be multiple related or unrelated reasons
	 * for a load to fail.
	 */
	 ...
	 *
	 * <p>
	 * Each root cause is logged separately to avoid throttling.
	 * {@link #printStackTrace()} will provide a more succinct overview of why
	 * the exception occurred, although it does not include complete stack
	 * traces.
	 */
	...
	public String getMessage() {
		return super.getMessage() + (dataClass != null ? ", " + dataClass : "")
				+ (dataSource != null ? ", " + dataSource : "") + (key != null ? ", " + key : "");
	}

	// Appendable throws, PrintWriter, PrintStream, and IndentedAppendable do
	// not, so this should
	// never happen.
	...
	private static void appendCausesWrapped(List<Exception> causes, Appendable appendable) throws IOException {
		...
		for (int i = 0; i < size; i++) {
			appendable.append("Cause (").append(String.valueOf(i + 1)).append(" of ").append(String.valueOf(size))
					.append("): ");

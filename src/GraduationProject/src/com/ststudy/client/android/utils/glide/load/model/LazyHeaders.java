 ...
 *
 * <p>
 * Ideally headers are constructed once and then re-used for multiple loads,
 * rather then being constructed individually for each load.
 * </p>
 *
 * <p>
...
public final class LazyHeaders implements Headers {
	 ...
	 *
	 * <p>
	 * This class may include default values for User-Agent and Accept-Encoding
	 * headers. These will be replaced by calls to either
	 * {@link #setHeader(String, LazyHeaderFactory)} or
	 * {@link #addHeader(String, String)}, even though
	 * {@link #addHeader(String, LazyHeaderFactory)} would usually append an
	 * additional value.
	 * </p>
	 */
	// PMD doesn't like the necessary static block to initialize
	// DEFAULT_HEADERS.
	public static final class Builder {
		...
		// Set Accept-Encoding header to do our best to avoid gzip since it's
		// both inefficient for
		// images and also makes it more difficult for us to detect and prevent
		// partial content
		// rendering. See #440.
		static {
			...
			if (!TextUtils.isEmpty(DEFAULT_USER_AGENT)) {
				temp.put(USER_AGENT_HEADER,
						Collections.<LazyHeaderFactory> singletonList(new StringHeaderFactory(DEFAULT_USER_AGENT)));
			}
			temp.put(ENCODING_HEADER,
					Collections.<LazyHeaderFactory> singletonList(new StringHeaderFactory(DEFAULT_ENCODING)));
		 ...
		 * value requires I/O (ie an oauth token).
		 * </p>
		 *
		 ...
		 *
		 * <p>
		 * Headers may have multiple values whose order is defined by the order
		...
		/**
		 * Replaces all existing {@link LazyHeaderFactory LazyHeaderFactorys}
		 * for the given key with the given {@link LazyHeaderFactory}.
		 *
		 * <p>
		 * If the given value is {@code null}, the header at the given key will
		 * be removed.
		 * </p>
		 *
		 * <p>
